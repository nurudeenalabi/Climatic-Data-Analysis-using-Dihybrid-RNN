...
# predictions = data_frame(best_lstm_preds_exp,best_gru_preds_exp)
predictions <- tibble(best_lstm_preds_exp, best_gru_preds_exp)
# Predicted values for LSTM
predicted_lstm <- best_lstm_preds_exp

# Predicted values for GRU
predicted_gru <- best_gru_preds_exp

# Combine into a single data frame
predictions_df_combined <- data.frame(
  Index = seq_along(predicted_lstm),
  Predicted_LSTM = predicted_lstm,
  Predicted_GRU = predicted_gru
)
...
# 
# # Plotting LSTM residuals
# lstm_residual_plot <- ggplot(residuals_df_lstm, aes(x = Index, y = Residuals)) +
#   geom_line(color = "purple") +
#   labs(title = "LSTM: Residuals", x = "Index", y = "Residuals")
# 
# # Residuals for GRU
...
# 
# # Plotting GRU residuals
# gru_residual_plot <- ggplot(residuals_df_gru, aes(x = Index, y = Residuals)) +
#   geom_line(color = "orange") +
#   labs(title = "GRU: Residuals", x = "Index", y = "Residuals")
# 
# # Display the plots
# print(lstm_residual_plot)
# print(gru_residual_plot)
...
# 
# # Plotting LSTM residuals histogram
...
# 
# # Plotting GRU residuals histogram
...
hybrid_eval <- c(Loss = 0.01546733, MAE = 0.08719177)
...
# Assuming y_test, x_train, y_train, x_test are already defined in your environment
...
lstm_eval <- c(Loss = 0.05262252, MAE = 0.16555671)
gru_eval <- c(Loss = 0.05917004, MAE = 0.18611154)
hybrid_eval <- c(Loss = 0.01725331, MAE = 0.09072758)
...
p1 <- ggplot(results, aes(x = Index)) +
  geom_line(aes(y = Actual, colour = "Actual"), size = 1) +
  geom_line(aes(y = Predicted, colour = "Predicted"), size = 1) +
  labs(title = "Actual vs Predicted Values (Original Scale)",
       x = "Index",
       y = "Values",
       colour = "Legend") +
  theme_minimal() +
