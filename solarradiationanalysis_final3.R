...
results_by_city <- climatedata %>%
  group_by(city) %>%
  group_modify(~ {
    if (nrow(.x) < 2) return(tibble(city = unique(.x$city), lstm_mae = NA, gru_mae = NA))
    
    train_indices <- sample(seq_len(nrow(.x)), size = 0.8 * nrow(.x))
    train_data <- .x[train_indices, ]
    test_data <- .x[-train_indices, ]
    
    if (nrow(train_data) == 0 || nrow(test_data) == 0) return(tibble(city = unique(.x$city), lstm_mae = NA, gru_mae = NA))
    ...
    if (nrow(x_train) == 0 || nrow(y_train) == 0 || nrow(x_test) == 0 || nrow(y_test) == 0) return(tibble(city = unique(.x$city), lstm_mae = NA, gru_mae = NA))
    ...
    tibble(city = unique(.x$city), lstm_mae = lstm_result$evaluation[[2]], gru_mae = gru_result$evaluation[[2]])
  })
