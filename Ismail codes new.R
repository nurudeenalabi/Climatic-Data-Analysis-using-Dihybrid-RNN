...
build_gru_model <- function(units = 150, learning_rate = 0.0001, optimizer = 'nadam', activation = 'tanh', recurrent_activation = 'hard_tanh', batch_size = 2, epochs = 150){
  input_layer <- layer_input(shape = c(ncol(x_train), 1))
...
ggplot(residuals_df_lstm, aes(x = Residuals)) +
...
ggplot(residuals_df_gru, aes(x = Residuals)) +
