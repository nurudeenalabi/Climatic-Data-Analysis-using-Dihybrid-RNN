...
climatedata <- decompose_and_add_features(climatedata, "rf", freq = 365)
...
x_train <- as.matrix(train_data %>% select(tmin, tmax, sh, ws, rh, ep, sr, trend, seasonal, remainder))
y_train <- as.matrix(train_data$rf)
x_test <- as.matrix(test_data %>% select(tmin, tmax, sh, ws, rh, ep, sr, trend, seasonal, remainder))
y_test <- as.matrix(test_data$rf)
