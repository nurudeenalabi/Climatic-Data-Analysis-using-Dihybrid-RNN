...
results_by_city <- climatedata %>%
  group_by(city) %>%
  do({
    train_indices <- sample(seq_len(nrow(.)), size = 0.8 * nrow(.))
    train_data <- .[train_indices, ]
    test_data <- .[-train_indices, ]
    ...
    list(city = unique(.$City), lstm_mae = lstm_result$evaluation[[2]], gru_mae = gru_result$evaluation[[2]])
  })
