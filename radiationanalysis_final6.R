...
# Load necessary libraries
library(tidyverse)
raw_climatedata <- read.csv("climatedata_new.csv", stringsAsFactors = FALSE)
# Required Libraries
...
ggplot(raw_climatedata, aes(x = 1:nrow(raw_climatedata), y = sr, color = city, group = city)) +
  geom_line() +
  labs(title = "Solar Radiation Time Plot",
       x = "City and Year",
       y = "Solar Radiation ((ml)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_x_continuous(breaks = seq(1, nrow(raw_climatedata), by = 6), labels = raw_climatedata$CityYear[seq(1, nrow(raw_climatedata), by = 6)]) +
  geom_vline(xintercept = city_breaks, linetype = "dashed", color = "red") +
  scale_color_manual(values = c("Ilorin" = "black", "Sokoto" = "green", "Enugu" = "brown"))
# Load the data
...
build_lstm_model <- function(units = 150, learning_rate = 0.00001, optimizer = 'nadam', activation = 'relu', recurrent_activation = 'gelu', batch_size = 16, epochs = 150) {
...
build_gru_model <- function(units = 150, learning_rate = 0.00001, optimizer = 'nadam', activation = 'tanh', recurrent_activation = 'hard_tanh', batch_size = 16, epochs = 150) {
  input_layer <- layer_input(shape = c(ncol(x_train), 1))
...
hybrid_eval_hybrid <- c(loss , mean_absolute_error)
...
hybrid_eval <- c(hybrid_eval_hybrid, RMSE = rmse_value, R2 = r2_value)
print(hybrid_eval)
