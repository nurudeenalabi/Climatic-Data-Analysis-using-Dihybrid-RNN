...
raw_climatedata <- read.csv("climatedatanew2.csv", stringsAsFactors = FALSE)
...
ggplot(raw_climatedata, aes(x = 1:nrow(raw_climatedata), y = sr, color = city, group = city)) +
  geom_line() +
  labs(title = "Time Plot of Solar Radiation over Sokoto, Ilorin and Enugu, Maiduguri, Ikeja and Port Harcourt",
       x = "Period",
       y = "Solar Radiation ((ml)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_vline(xintercept = city_breaks, liAnetype = "dashed", color = "red") +
  scale_color_manual(values = c("Sokoto" = "black", "Maiduguri" = "green", "Ilorin" = "purple", "Ikeja" = "orange", "Enugu" = "brown", "PortHarcourt" = "blue"))
...
# Build LSTM GRU models function with fixed parameters
swish <- function(x) {
  x * keras::k_sigmoid(x)
}
leaky_relu <- function(x, alpha = 0.1) {
  keras::k_relu(x, alpha = alpha)
}

build_lstm_model <- function(units = 150, learning_rate = 0.00001, optimizer = 'nadam', activation = 'relu', recurrent_activation = swish, batch_size = 16, epochs = 150) {
...
build_gru_model <- function(units = 150, learning_rate = 0.00001, optimizer = 'nadam', activation = 'tanh', recurrent_activation = leaky_relu, batch_size = 16, epochs = 150) {
  input_layer <- layer_input(shape = c(ncol(x_train), 1))
...
# print(gru_eval);print(lstm_eval)
# # Assuming you have the keras library loaded
# library(keras)

# Train and capture LSTM model history
best_lstm_model_info <- build_lstm_model()
...
print(lstm_history)
# lstm_eval <- lstm_history %>% evaluate(x_test, y_test, verbose = 2)
best_lstm_preds <- best_lstm_model_info$model %>% predict(x_test)
# print(lstm_eval)
# Train and capture GRU model history
best_gru_model_info <- build_gru_model()
...
# gru_eval <- gru_history %>% evaluate(x_test, y_test, verbose = 2)
best_gru_preds <- best_gru_model_info$model %>% predict(x_test)
print(gru_history);print(lstm_history)
...
combined_residual_plots <- plot_grid(
  lstm_residual_hist + theme(plot.title = element_text(hjust = 0.2)),
  gru_residual_hist + theme(plot.title = element_text(hjust = 0.2)),
  labels = c("A", "B"),
  ncol = 2
)
...
p1 <- ggplot(results, aes(x = Index)) +
  geom_line(aes(y = Actual, colour = "Actual"), size = 0.05) +
  geom_line(aes(y = Predicted, colour = "Predicted"), size = 0.08) +
...
##### Data preparation ####
set.seed(123)
train_indices <- sample(seq_len(nrow(climatedata)), size = 1.0 * nrow(climatedata))
train_data <- climatedata[train_indices, ]
test_data <- climatedata[train_indices, ]
...
# Line plot for actual vs predicted values
p3 <- ggplot(results, aes(x = Index)) +
  geom_line(aes(y = Actual, colour = "Actual"), size = 0.5) +
  geom_line(aes(y = Predicted, colour = "Predicted"), size = 0.5) +
  labs(title = "Actual vs Predicted Values",
       x = "Index",
       y = "Values",
       colour = "Legend") +
...
# Histogram of residuals
p4 <- ggplot(residuals_df, aes(x = Residuals)) +
  geom_histogram(fill = "purple", bins = 30, alpha = 0.7) +
  labs(title = "Residuals Histogram",
       x = "Residuals",
       y = "Frequency") +
...
# # To evaluate the performance of the hybrid model by cities and obtain predictions for each city
# 
# train_hybrid_model <- function(data) {
#   # Preprocess data
#   data <- climatedata %>%
#     filter(!is.na(year)) %>%
#     arrange(year)
...
#   x <- as.matrix(data[, c("tmin", "tmax", "sh", "ws", "rh", "ep", "rf")])
#   y <- data$sr
# 
...
#     x, y,
#     epochs = 150,
#     batch_size = 16, recurrent_activation = 'gelu',
...
# data_by_city <- split(climatedata, climatedata$city)
# 
# # Train models for each city
# models_by_city <- lapply(data_by_city, train_hybrid_model)
...
#   x <- array_reshape(x, c(nrow(x), 1, ncol(x)))
#   
#   metrics <- model %>% evaluate(x, y, verbose = 2)
#   list(loss = metrics[[1]], mae = metrics[[2]])
# }
# 
# performance_by_city <- lapply(names(models_by_city), function(city) {
#   data <- data_by_city[[city]]
#   model <- models_by_city[[city]]$model
#   metrics <- evaluate_performance(model, data)
#   tibble(city = city, loss = metrics$loss, mae = metrics$mae)
# }) %>% bind_rows()
# 
# print(performance_by_city)
# 
...
#   original_scale_predictions <- exp(predictions) - 1
#   data %>% mutate(predicted_sr = original_scale_predictions)
# }
# 
# predictions_by_city <- lapply(names(models_by_city), function(city) {
#   data <- data_by_city[[city]]
#   model <- models_by_city[[city]]$model
#   predictions <- make_predictions(model, data)
#   predictions %>% mutate(city = city)
# }) %>% bind_rows()
# 
# head(predictions_by_city);tail(predictions_by_city)
# 
# # Plotting the predictions against actual values
# plot_predictions <- function(data, city) {
#   ggplot(data, aes(x = year)) +
#     geom_line(aes(y = exp(sr) - 1, color = "Actual", group = 1)) +
#     geom_line(aes(y = predicted_sr, color = "Predicted", group = 1)) +
#     labs(title = paste("Actual vs Predicted SR for", city), y = "Solar Radiation (SR)", color = "Legend") +
#     theme_minimal()
# }
# 
# plots <- lapply(unique(predictions_by_city$city), function(city) {
#   data <- predictions_by_city %>% filter(city == !!city)
#   plot_predictions(data, city)
# })
# 
